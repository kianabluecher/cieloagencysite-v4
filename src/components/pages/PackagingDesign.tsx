...
import { packagingDesignFAQs } from '../../utils/faq-data';

interface PackagingDesignProps {
  onNavigate: (page: string) => void;
}

export function PackagingDesign({ onNavigate }: PackagingDesignProps) {
  return (
    <div className="min-h-screen bg-neutral-950">
      <section className="relative px-6 pt-32 pb-20 overflow-hidden min-h-[80vh] flex items-center">
        <div className="absolute inset-0 -top-32">
          <ImageWithFallback 
            src="https://images.unsplash.com/photo-1709090844277-f737b3d6ffcf?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxsdXh1cnklMjBwYWNrYWdpbmclMjBkZXNpZ258ZW58MXx8fHwxNzYwODk5MTk2fDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral"
            alt="Hero background" 
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full flex flex-col h-full justify-between">
          <div className="inline-block mb-12">
            <p className="font-['Geist_Mono'] text-[#7d8187] text-xs tracking-[1.4px] uppercase">[ Packaging Design ]</p>
          </div>

          <h1 className="text-5xl md:text-7xl lg:text-8xl tracking-tight leading-none mb-auto">
            <span className="block bg-gradient-to-r from-white via-white to-white/80 bg-clip-text text-transparent">First impressions</span>
            <span className="block bg-gradient-to-r from-white via-white to-white/80 bg-clip-text text-transparent">are profitable</span>
          </h1>

          <div className="flex justify-end mt-8">
            <p className="text-[#7d8187] max-w-md text-right leading-relaxed text-[12px] mt-8">
              70% of purchasing decisions happen at shelf. 100% of premium brands nail their packaging.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-20 border-t border-[#1f2228]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
            {[
              { title: 'Primary packaging that pops', description: 'Eye-catching designs that stand out on crowded shelves' },
              { title: 'Unboxing experiences that go viral', description: 'Memorable packaging that customers want to share' },
              { title: 'Sustainable solutions that sell', description: 'Eco-friendly packaging that doesn\'t compromise on luxury' },
            ].map((item, index) => (
      ...
      <section className="px-6 py-20 border-t border-[#1f2228]">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16">
            <h2 className="text-4xl md:text-5xl text-white tracking-tight">Our Network</h2>
            <p className="text-xl text-[#7d8187] max-w-2xl text-[15px]">
              Global production partners. Sustainable material sourcing. Prototype to production pipeline. Cost engineering included.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            {[
              { title: 'Concept & Strategy', description: 'Understanding your brand positioning and designing packaging that reflects premium quality.' },
              { title: 'Design Development', description: 'Creating multiple design concepts with structural innovation and visual impact.' },
              { title: 'Material Selection', description: 'Sourcing sustainable, premium materials that align with your brand values.' },
              { title: 'Production Management', description: 'Overseeing manufacturing from prototype to final production with quality control.' },
            ].map((item, index) => (
      ...
      <section className="relative px-6 py-32 border-t border-[#1f2228] overflow-hidden">
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-8">
          <h2 className="text-4xl md:text-5xl lg:text-6xl text-white tracking-tight leading-tight">Ready to elevate your packaging?</h2>
          <p className="text-2xl text-[#7d8187]">Let's create packaging worth sharing.</p>
      ...
      {/* FAQ Section */}
      <FAQSection faqs={packagingDesignFAQs} />
    </div>
  );
}