...
function XButton({ children, primary = false, onClick }: { children: React.ReactNode, primary?: boolean, onClick?: () => void }) {
  return (
...
function HeroSection() {
  const handleGetStarted = () => {
    window.open('https://wix.to/G7TbMyb', '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <div className="relative w-full min-h-[90vh] flex flex-col items-center justify-center overflow-hidden bg-black text-white pt-20 border-b border-[#222]">
        {/* Background Image - Full bleed, no padding */}
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto flex flex-col items-center">
            ...
            <motion.div 
                ...
                className="space-y-6 mb-10 max-w-4xl"
            >
                <h1 className="text-6xl md:text-7xl lg:text-8xl font-semibold leading-[1.1] tracking-tighter">
                    The Next Frontier <br/> of Enterprise AI
                </h1>
                <p className="text-[#999] text-[18px] md:text-[20px] max-w-xl leading-relaxed">
                    Same process, custom to your target.
                </p>
            </motion.div>
            ...
            >
               <XButton primary onClick={handleGetStarted}>GROK 4.1 FAST, NOW AVAILABLE</XButton>
            </motion.div>
        </div>
    </div>
  );
        ...
        <div className="w-full bg-black text-white">
            <div className="max-w-[1440px] mx-auto">
                <div className="p-10 border-b border-[#222]">
                    ...
                    <h2 className="text-4xl font-medium tracking-tight mt-4 mb-2">What you'll learn</h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 border-l border-[#222]">
                    <CapabilityCard 
                        ...
                        description="We cover the fundamentals first, ensuring you have a solid foundation to build on."
                    />
                    <CapabilityCard 
                        icon={Zap}
                        title="Practical Tools"
                        description="Expand into practical tools, systems, and real business use cases tailored to your needs."
                    />
                    <CapabilityCard 
                        icon={RefreshCw}
                        title="Stay Ahead"
                        description="AI will keep evolving. This ensures you understand the basics and stay ahead instead of falling behind."
                    />
                </div>
            </div>
        </div>
...
function PricingCard({ title, price, features, isPro = false }: { title: string, price: string, features: string[], isPro?: boolean }) {
    ...
    return (
        <div className={`p-10 md:p-16 border-b border-r border-[#222] flex flex-col justify-between ${isPro ? 'bg-[#050505]' : ''}`}>
            <div>
                <div className="flex justify-between items-start mb-8">
                    <div>
                        ...
                        <p className="text-[#666] text-sm">{isPro ? '4-week program' : '1h Session'}</p>
                    </div>
                    {isPro && (
                ...
                <div className="mb-10">
                    ...
                    <div className="text-[#666] text-sm mt-1">{isPro ? 'one-time investment' : 'per session'}</div>
                </div>
            ...
            <XButton primary={isPro} onClick={handleSubscribe}>{isPro ? 'Get Started' : 'Book Session'}</XButton>
        </div>
    );
}
    ...
    return (
        <div className="w-full bg-black text-white border-t border-[#222]">
            <div className="max-w-[1440px] mx-auto border-l border-[#222]">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2">
                    <PricingCard 
                        title="Personalized AI Consulting"
                        price="$350"
                        features={[
                            'Beginner or advanced level', 
                            'Fundamentals & practical tools', 
                            'Real business use cases', 
                            'Stay ahead of AI evolution'
                        ]}
                    />
                    <PricingCard 
                        title="Live Guidance & Implementation"
                        price="$9,000"
                        features={[
                            '4-week live guidance & implementation',
                            '1 call per week',
                            'Tools setup & configuration',
                            'Combined marketing & brand development',
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
                    <div>
                        <h2 className="text-4xl md:text-[48px] font-medium leading-tight tracking-tight mb-8">
                            Deep dive with <br/> AI Consulting
                        </h2>
                        <p className="text-[#999] text-lg leading-relaxed max-w-md">
                            A personalized session built around your level, beginner or advanced. We cover the fundamentals first, then expand into practical tools, systems, and real business use cases.
                        </p>
                        
                        <div className="mt-12">
                             <XButton onClick={handleDiveDeep}>Book Now</XButton>
                        </div>
                    </div>
                    
                    <div className="border-t border-[#222]">
                        <FeatureRow label="Session Type" value="Personalized" />
                        <FeatureRow label="Level" value="Beginner to Advanced" />
                        <FeatureRow label="Focus" value="Practical & Business" />
...
export function AIConsulting({ onNavigate }: { onNavigate: (page: string) => void }) {
  return (
