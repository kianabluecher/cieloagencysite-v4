...
interface SmStartFooterProps {
  onNavigate: (page: string) => void;
}

export function SmStartFooter({ onNavigate }: SmStartFooterProps) {
  ...
  return (
    <footer className="relative border-t border-[rgba(31,34,40,0.5)] bg-neutral-950 overflow-hidden">
      {/* Simple Blue Gradient Glow from Bottom */}
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 lg:gap-8">
          ...
          <div className="col-span-2 sm:col-span-3 md:col-span-4 lg:col-span-1">
            ...
            <p className="text-xs leading-relaxed text-[#7d8187]">
          ...
          {/* Services */}
          <div>
            <h4 className="font-['Geist_Mono'] text-[10px] tracking-[1.2px] uppercase mb-4 text-[#7d8187]">
              Services
            </h4>
            <div className="space-y-3">
              {services.map((service, index) => (
                <button
                  ...
                  onClick={(e) => handleNavigate(e, service.page)}
                  className="block transition-colors text-left text-sm cursor-pointer text-white hover:text-[#7d8187]"
                >
                  {service.label}
                </button>
              ))}
              ...
              <div className="mt-4">
                <button 
                  ...
                  className="flex items-center gap-1.5 transition-colors text-left text-sm cursor-pointer text-white hover:text-[#7d8187]"
                >
                ...
                <div className={`mt-0 overflow-hidden transition-all duration-500 ease-in-out ${
                  ...
                  <div className="pt-3 space-y-0">
                    {additionalServices.map((service, index) => (
                      <div key={index}>
                        {index > 0 && (
                          <div className="h-px my-2 bg-[#1f2228]"></div>
                        )}
                        <button
                          onClick={(e) => handleNavigate(e, service.page)}
                          className="flex items-center gap-2 transition-colors text-left w-full text-sm cursor-pointer text-[#7d8187] hover:text-white"
                        >
                          {service.label}
                          {service.badge && (
            ...
            <div className="space-y-3">
              {company.map((item, index) => (
                <button
                  ...
                  className="flex items-center gap-2 transition-colors text-left text-sm cursor-pointer text-white hover:text-[#7d8187]"
                >
                  {item.label}
                  {item.badge && (
                  ...
                  <div className="pt-3 space-y-0">
                    {logins.map((login, index) => (
                      <div key={index}>
                        ...
                        <button
                          onClick={(e) => handleNavigate(e, login.page)}
                          className="flex items-center gap-2 transition-colors text-left w-full text-sm cursor-pointer text-[#7d8187] hover:text-white"
                        >
                          {login.label}
                        </button>
            ...
            <div className="space-y-3">
              {offersFreebies.map((item, index) => (
                item.url ? (
                  ...
                  <button
                    key={index}
                    onClick={(e) => item.page && handleNavigate(e, item.page)}
                    className="block transition-colors text-left text-sm cursor-pointer text-white hover:text-[#7d8187]"
                  >
                    {item.label}
                  </button>
            ...
            <div className="space-y-3">
              {brands.map((item, index) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 transition-colors text-sm cursor-pointer text-white hover:text-[#7d8187]"
                >
                  {item.label}
                  {item.badge && (
            ...
            <div className="space-y-3">
              {socials.map((item, index) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block transition-colors text-sm cursor-pointer text-white hover:text-[#7d8187]"
                >
                  {item.label}
                </a>
              ))}
        ...
        <div className="mt-16 pt-8 border-t border-[#1f2228]">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-8">
            <a
              href="mailto:hello@cielo.agency"
              className="transition-colors cursor-pointer text-white hover:text-[#7d8187]"
            >
              hello@cielo.agency
            </a>
            <button
              onClick={(e) => handleNavigate(e, 'pricing-download')}
              className="px-6 py-2.5 rounded-full border border-[rgba(255,255,255,0.3)] text-white hover:border-white font-['Geist_Mono'] text-xs tracking-[1.4px] uppercase transition-colors cursor-pointer"
            >
              Get a Quote
            </button>
            <a 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 transition-colors font-['Geist_Mono'] text-xs tracking-[1.4px] uppercase underline underline-offset-4 cursor-pointer text-white hover:text-[#7d8187]"
            >
        ...
        <div className="mt-16 pt-8 border-t border-[#1f2228]">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <p className="text-sm font-['Geist_Mono'] tracking-wider text-[#7d8187]">
